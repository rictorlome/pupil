...
func StatelessMove(t *testing.T, test TestStatelessUpdate) {
	p := parseFen(test.initial)
	endPos := parseFen(test.end)
	move := p.parseMove(test.uci)
	k := p.state.key

	// DO MOVE
	p.doMove(move, &StateInfo{})
	if generateFen(p) != test.end {
		t.Errorf("Do move: %v, Expected not equal to actual:\n%v %v\n", test.uci, test.end, generateFen(p))
	}
	if k == p.state.key {
		t.Errorf("Do move: %v, Key did not update.", test.uci)
	}
	if p.state.key != endPos.state.key {
		t.Errorf("Do move: %v, Key did not update correctly. Expected not equal to actual:\n%v %v\n", test.uci, endPos.state.key, p.state.key)
	}
	// UNDO MOVE
	p.undoMove(move)
	if generateFen(p) != test.initial {
		t.Errorf("Undo move: %v, Expected not equal to actual:\n%v %v\n", test.uci, test.initial, generateFen(p))
	}
	if k != p.state.key {
		t.Errorf("Undo move: %v, Key did not reset on undoMove. Expected not equal to actual:\n%v %v\n", test.uci, k, p.state.key)
	}
}
